...
export async function POST(request: Request) {
  ...
  const upstream = await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({...record,authority:true,source:'autocomp-corp'}),cache:'no-store',signal:AbortSignal.timeout(8000)});
